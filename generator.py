...
def complete_mask_map(mini_mapX):
    ...
    H = ref_point[3]-ref_point[1]
    L = ref_point[2]-ref_point[0]

    print("taille", ref_point)
    mini_mapX.create_like_file()

    # LOW MAP explication algorythme:
    #   * see draw on ipad
    ratio_remplissable = 1
    seuil = ratio_remplissable * 20
    ...
    for i in list(range(H)):
        ...
        # draw two vertical lines and add link if pixel is a frontier.
        #1
        if mask_map[ref_point[0],ref_point[1]+i,0] != 0:
            # we need to check if this link is empty of full, if empty go link.
            if ma_map_global[ref_point[1]+i,ref_point[0]] == 255:
                link = str(0)+" "+str(i)+" "+str(mask_map[ref_point[0],ref_point[1]+i,1])+" "+str(mask_map[ref_point[0],ref_point[1]+i,2])+" "+str(mask_map[ref_point[0],ref_point[1]+i,0])
                mini_mapX.add_link(link,1)
                reverse_link = str(mask_map[ref_point[0],ref_point[1]+i,1])+" "+str(mask_map[ref_point[0],ref_point[1]+i,2])+" "+str(0)+" "+str(i)+" "+str(mini_mapX.map_number)
                list_map[mask_map[ref_point[0],ref_point[1]+i,0]-1].add_link(reverse_link,1)
                ...
                if compteur_horizontal_A >= seuil:
                    ...
                    y_coord_ref_low = int(i//20)
                    xLINK_coord_ref_low = int(mask_map[ref_point[0],ref_point[1]+i,1]//20)
                    yLINK_coord_ref_low = int(mask_map[ref_point[0],ref_point[1]+i,2]//20)
                    to_map = int(mask_map[ref_point[0],ref_point[1]+i,0])
                    mini_mapX.add_link(str(x_coord_ref_low)+" "+str(y_coord_ref_low)+" "+str(xLINK_coord_ref_low)+" "+str(yLINK_coord_ref_low)+" "+str(to_map)+" ",0)
                    list_map[mask_map[ref_point[0],ref_point[1]+i,0]-1].add_link(str(xLINK_coord_ref_low)+" "+str(yLINK_coord_ref_low)+" "+str(x_coord_ref_low)+" "+str(y_coord_ref_low)+" "+str(mini_mapX.map_number),0)

        mask_map[ref_point[0],ref_point[1]+i,0] = mini_map.nombre_mini_map
        mask_map[ref_point[0],ref_point[1]+i,1] = 0 #x
        mask_map[ref_point[0],ref_point[1]+i,2] = i #y
        #2
        if mask_map[ref_point[2],ref_point[1]+i,0] != 0:
            if ma_map_global[ref_point[1]+i,ref_point[2]] == 255:
                link = str(H-1)+" "+str(i)+" "+str(mask_map[ref_point[2],ref_point[1]+i,1])+" "+str(mask_map[ref_point[2],ref_point[1]+i,2])+" "+str(mask_map[ref_point[2],ref_point[1]+i,0])
                mini_mapX.add_link(link,1)
                reverse_link = str(mask_map[ref_point[2],ref_point[1]+i,1])+" "+str(mask_map[ref_point[2],ref_point[1]+i,2])+" "+str(H-1)+" "+str(i)+" "+str(mini_mapX.map_number)
                list_map[mask_map[ref_point[2],ref_point[1]+i,0]-1].add_link(reverse_link,1)
                ...
                if compteur_horizontal_B >= seuil:
                    ...
                    y_coord_ref_low = int(i//20)
                    xLINK_coord_ref_low = int(mask_map[ref_point[2],ref_point[1]+i,1]//20)
                    yLINK_coord_ref_low = int(mask_map[ref_point[2],ref_point[1]+i,2]//20)
                    to_map = int(mask_map[ref_point[2],ref_point[1]+i,0])
                    mini_mapX.add_link(str(x_coord_ref_low)+" "+str(y_coord_ref_low)+" "+str(xLINK_coord_ref_low)+" "+str(yLINK_coord_ref_low)+" "+str(to_map)+" ",0)
                    list_map[mask_map[ref_point[2],ref_point[1]+i,0]-1].add_link(str(xLINK_coord_ref_low)+" "+str(yLINK_coord_ref_low)+" "+str(x_coord_ref_low)+" "+str(y_coord_ref_low)+" "+str(mini_mapX.map_number),0)


        mask_map[ref_point[2],ref_point[1]+i,0] = mini_map.nombre_mini_map
        mask_map[ref_point[2],ref_point[1]+i,1] = H-1
        mask_map[ref_point[2],ref_point[1]+i,2] = i
    ...
    for i in list(range(L)):
        ...
        # draw two horizontal lines and add link if pixel is a frontier.
        #3
        if (mask_map[ref_point[0]+i,ref_point[1],0] != 0) and (mask_map[ref_point[0]+i,ref_point[1],0] != mini_map.nombre_mini_map):
            if ma_map_global[ref_point[1],ref_point[0]+i] == 255:
                link = str(i)+" "+str(0)+" "+str(mask_map[ref_point[0]+i,ref_point[1],1])+" "+str(mask_map[ref_point[0]+i,ref_point[1],2])+" "+str(mask_map[ref_point[0]+i,ref_point[1],0])
                mini_mapX.add_link(link,1)
                reverse_link = str(mask_map[ref_point[0]+i,ref_point[1],1])+" "+str(mask_map[ref_point[0]+i,ref_point[1],2])+" "+str(i)+" "+str(0)+" "+str(mini_mapX.map_number)
                list_map[mask_map[ref_point[0]+i,ref_point[1],0]-1].add_link(reverse_link,1)
                ...
                if compteur_vertical_A >= seuil:
                    compteur_vertical_A = 0
                    # create new link in LOW MAP.
                    x_coord_ref_low = int(i//20)
                    y_coord_ref_low = int(0//20)
                    xLINK_coord_ref_low = int(mask_map[ref_point[0]+i,ref_point[1],1]//20)
                    yLINK_coord_ref_low = int(mask_map[ref_point[0]+i,ref_point[1],2]//20)
                    to_map = int(mask_map[ref_point[0]+i,ref_point[1],0])
                    mini_mapX.add_link(str(x_coord_ref_low)+" "+str(y_coord_ref_low)+" "+str(xLINK_coord_ref_low)+" "+str(yLINK_coord_ref_low)+" "+str(to_map)+" ",0)
                    list_map[mask_map[ref_point[0]+i,ref_point[1],0]-1].add_link(str(xLINK_coord_ref_low)+" "+str(yLINK_coord_ref_low)+" "+str(x_coord_ref_low)+" "+str(y_coord_ref_low)+" "+str(mini_mapX.map_number),0)
        
        mask_map[ref_point[0]+i,ref_point[1],0] = mini_map.nombre_mini_map
        mask_map[ref_point[0]+i,ref_point[1],1] = i
        mask_map[ref_point[0]+i,ref_point[1],2] = 0

        #4
        if (mask_map[ref_point[0]+i,ref_point[3],0] != 0) and (mask_map[ref_point[0]+i,ref_point[3],0] != mini_map.nombre_mini_map):
            if ma_map_global[ref_point[3],ref_point[0]+i] == 255:
                link = str(i)+" "+str(L-1)+" "+str(mask_map[ref_point[0]+i,ref_point[3],1])+" "+str(mask_map[ref_point[0]+i,ref_point[3],2])+" "+str(mask_map[ref_point[0]+i,ref_point[3],0])
                mini_mapX.add_link(link,1)
                reverse_link = str(mask_map[ref_point[0]+i,ref_point[3],1])+" "+str(mask_map[ref_point[0]+i,ref_point[3],2])+" "+str(i)+" "+str(L-1)+" "+str(mini_mapX.map_number)
                list_map[mask_map[ref_point[0]+i,ref_point[3],0]-1].add_link(reverse_link,1)
                ...
                if compteur_vertical_B >= seuil:
                    ...
                    xLINK_coord_ref_low = int(mask_map[ref_point[0]+i,ref_point[3],1]//20)
                    yLINK_coord_ref_low = int(mask_map[ref_point[0]+i,ref_point[3],2]//20)
                    to_map = int(mask_map[ref_point[0]+i,ref_point[3],0])
                    mini_mapX.add_link(str(x_coord_ref_low)+" "+str(y_coord_ref_low)+" "+str(xLINK_coord_ref_low)+" "+str(yLINK_coord_ref_low)+" "+str(to_map)+" ",0)
                    list_map[mask_map[ref_point[0]+i,ref_point[3],0]-1].add_link(str(xLINK_coord_ref_low)+" "+str(yLINK_coord_ref_low)+" "+str(x_coord_ref_low)+" "+str(y_coord_ref_low)+" "+str(mini_mapX.map_number),0)

        mask_map[ref_point[0]+i,ref_point[3],0] = mini_map.nombre_mini_map
        mask_map[ref_point[0]+i,ref_point[3],1] = i
        mask_map[ref_point[0]+i,ref_point[3],2] = L-1
...
if __name__ == "__main__":
    clean_graph_file()
    ma_map = import_image_and_transform("image/real2.jpg")
    ma_map_global = ma_map
    ma_map_affichage = ma_map.copy()
    mask_map = np.zeros((ma_map_global.shape[0], ma_map_global.shape[1], 3),dtype=np.uint8)
